...
const DEFAULT_CONFIG = {
  ...
  indexTtl: parseInt(process.env.RESPONSE_CACHE_INDEX_TTL || '7200'), // 2 hours
...
const INDEX_KEY = 'resp_cache:index';
...
class ResponseCacheService {
  ...
  async checkCache(
    ...
    try {
      ...
      const indexData = await redis.get(INDEX_KEY);
      ...
      if (bestMatch) {
        const cachedData = await redis.get(`${CACHE_PREFIX}${bestMatch.key}`);
        if (cachedData) {
          const cached: CachedResponse = JSON.parse(cachedData);
          // Increment hit count
          cached.hitCount++;
          await redis.set(
            `${CACHE_PREFIX}${bestMatch.key}`,
      ...
      try {
        ...
        if (index.length > DEFAULT_CONFIG.maxCacheSize) {
          ...
          for (const entry of toRemove) {
            await redis.del(`${CACHE_PREFIX}${entry.key}`);
          }
        }

        await redis.set(INDEX_KEY, JSON.stringify(index), 'EX', DEFAULT_CONFIG.indexTtl);
  ...
  async invalidateByPattern(pattern: string): Promise<number> {
    ...
    try {
      ...
      await redis.set(INDEX_KEY, JSON.stringify(remaining), 'EX', DEFAULT_CONFIG.indexTtl);

      log.info('Cache invalidated by pattern', { pattern, removed: toRemove.length });
   ...
   * Get cache stats.
   *
   * NOTE: `createdAt` lives inside the individual `CachedResponse` object,
   * NOT in the lightweight index entries.  We fetch the first entry from
   * Redis to get a real timestamp instead of reading a field that never
   * existed on the index shape.
   */
  async getStats(): Promise<{ size: number; maxSize: number; oldestEntry: number | null }> {
    try {
      const indexData = await redis.get(INDEX_KEY);
      ...
      if (index.length > 0) {
        try {
          const firstData = await redis.get(`${CACHE_PREFIX}${index[0].key}`);
          if (firstData) {
